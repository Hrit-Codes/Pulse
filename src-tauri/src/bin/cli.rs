...
use pulse_lib::{discover::{DeviceInfo, broadcaster::broadcast_discover, listener::listen_for_discover}, storage::TransferStore, transfer::{receiver::receive_file, sender::send_file}};
...
async fn main(){
    ...
    if let Ok(res) = store{
        if let Ok(val) = res.chunk_dir_for("rochakadhi123transfer"){
            println!("{:?}",val)
        }
    }
}
